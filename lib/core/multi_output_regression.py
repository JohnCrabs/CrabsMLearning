...
def MachineLearning(input_data, output_data, name='', dnn_LactFunc='selu', dnn_OactFunc='sigmoid',
                    dnn_loss='mse', lstm_LactFunc='tanh', lstm_DactFunc='sigmoid', css_LactFunc='sigmoid',
                    lstm_loss='mean_squared_error', lstm_optimizer='adam', seq_div=1, epochs=100, batch_size=100,
                    min_lr=0.001, dropout_percentage=0.1, kernel_size=3):
    ...
    inputDataToUse = input_data
    outputDataToUse = output_data

    input_shape = inputDataToUse.shape
    output_shape = outputDataToUse.shape
    print(input_shape)
    print(output_shape)
    ...
    lr1 = keras.layers.Dense(input_shape[1] * 2, activation=dnn_LactFunc)(inputs)
    do1 = keras.layers.Dropout(0.2)(lr1)
    lr2 = keras.layers.Dense(input_shape[1], activation=dnn_LactFunc)(do1)  # decoder
    lr3 = keras.layers.Dense(input_shape[1] * 2, activation=dnn_LactFunc)(lr2)
    do2 = keras.layers.Dropout(0.2)(lr3)
    outputs = keras.layers.Dense(output_shape[1], activation=dnn_OactFunc)(do2)
    # outputs = keras.layers.Reshape([28, 28])(lr4)
    DNN = keras.models.Model(inputs, outputs)
    DNN.compile(loss=dnn_loss, optimizer=keras.optimizers.SGD(lr=1.5))

    # LSTM model here
    dense_layer_size = int(input_shape[1] * 0.8)
    if dense_layer_size % 2 == 1:
        dense_layer_size += 1
    LSTM_model = keras.Sequential()
    LSTM_model.add(keras.layers.InputLayer(input_shape=(input_shape[1], 1)))
    ...
    LSTM_model.add(keras.layers.Reshape((output_shape[1], -1)))
    ...
    # Sequential
    inputSeqData = input_data.reshape(input_shape[0], seq_div, int(input_shape[1] / seq_div))
    outputSeqData = output_data.reshape(output_shape[0], seq_div, int(output_shape[1] / seq_div))

    input_seq_shape = inputSeqData.shape
    output_seq_shape = outputSeqData.shape
    ...
    tmpInputs = inputDataToUse
    tmpOutputs = outputDataToUse
    # create train, validation and test sets indexes
    randomIndexes = \
        np.random.permutation(tmpInputs.shape[0])
    # keep 75% as training, 5% as validation and 20% as training
    trainIdxs = np.sort(randomIndexes[:int(np.round(0.75 * len(randomIndexes)))])
    valIdxs = np.sort(
        randomIndexes[int(np.round(0.75 * len(randomIndexes)) + 1): int(np.round(0.8 * len(randomIndexes)))])
    testIdxs = np.sort(randomIndexes[int(np.round(0.8 * len(randomIndexes))) + 1:])
    ...
    for modelName, estimator in availableEstimators.items():
        ...
        if modelName is FLAG_LSTM:
            estimator.fit(np.expand_dims(tmpInputs[trainIdxs, :], axis=2),
                          np.expand_dims(tmpOutputs[trainIdxs, :], axis=2),
            ...
            estimator.fit(inputSeqData[trainIdxs, :, :],
                          outputSeqData[trainIdxs, :, :],
                          batch_size=batch_size, shuffle=True, epochs=deepModelsTrainingEpochs, verbose=1,
                          callbacks=callbacksOptions,
                          validation_data=(inputSeqData[valIdxs, :, :], outputSeqData[valIdxs, :, :]))
            print(' ... training completed.')
            # calculate models outputs for all entries
            trainSetEstimatorPredictions = estimator.predict(inputSeqData[trainIdxs, :, :])
            ...
            testSetEstimatorPredictions = estimator.predict(inputSeqData[testIdxs, :, :])
        ...
        errorPerObservationPointMAETrain = np.zeros(tmpOutputs.shape[1])
        errorPerObservationPointMSETrain = np.zeros(tmpOutputs.shape[1])
        errorPerObservationPointMaxErrorTrain = np.zeros(tmpOutputs.shape[1])

        errorPerObservationPointMAETest = np.zeros(tmpOutputs.shape[1])
        errorPerObservationPointMSETest = np.zeros(tmpOutputs.shape[1])
        errorPerObservationPointMaxErrorTest = np.zeros(tmpOutputs.shape[1])

        # calculate related errors
        for observationPointIdx in range(0, tmpOutputs.shape[1]):
            # normalized first
            errorPerObservationPointMAETrain[observationPointIdx] = \
                mean_absolute_error(tmpOutputs[trainIdxs, observationPointIdx],
                                    trainSetEstimatorPredictions[:, observationPointIdx])
            errorPerObservationPointMSETrain[observationPointIdx] = \
                mean_squared_error(tmpOutputs[trainIdxs, observationPointIdx],
                                   trainSetEstimatorPredictions[:, observationPointIdx])
            errorPerObservationPointMaxErrorTrain[observationPointIdx] = \
                max_error(tmpOutputs[trainIdxs, observationPointIdx],
                          trainSetEstimatorPredictions[:, observationPointIdx])

            errorPerObservationPointMAETest[observationPointIdx] = \
                mean_absolute_error(tmpOutputs[testIdxs, observationPointIdx],
                                    testSetEstimatorPredictions[:, observationPointIdx])
            errorPerObservationPointMSETest[observationPointIdx] = \
                mean_squared_error(tmpOutputs[testIdxs, observationPointIdx],
                                   testSetEstimatorPredictions[:, observationPointIdx])
            errorPerObservationPointMaxErrorTest[observationPointIdx] = \
                max_error(tmpOutputs[testIdxs, observationPointIdx],
                          testSetEstimatorPredictions[:, observationPointIdx])
